...
/*
    *
    Создание GIN-индекса на массивы в качестве эксперимента.

    В таблицу пользователей добавила столбец photo_list, куда с помощью функции нагенерила массивы чисел,
    чтобы потестировать, как работает GIN-index с массивами. В таблице более 3000 записей.
    Сначала пробовала потестить на таблице товаров, где 25 строк только, индекс не подхватывается.
*/

-- Создаёт индекс:
CREATE INDEX photo_ids_idx 
ON users USING GIN (photo_list);

-- Запрос, который может исполняться на массивах (проверяем, содержится ли какой-то массив в массивах таблицы):
select *
from users
where photo_list @> ARRAY[100,99];

-- Результат EXPLAIN:
Bitmap Heap Scan on dicts.users  (cost=12.08..47.75 rows=11 width=136) (actual time=0.053..0.055 rows=1 loops=1)
    Output: id, dttmcr, last_name, first_name, middle_name, birth_date, email, password_hash, phone, gender, status_id, photo_list
    Recheck Cond: (users.photo_list @> '{100,99}'::integer[])
    Heap Blocks: exact=1
    ->  Bitmap Index Scan on photo_ids_idx  (cost=0.00..12.08 rows=11 width=0) (actual time=0.049..0.049 rows=1 loops=1)
        Index Cond: (users.photo_list @> '{100,99}'::integer[])
Planning Time: 0.389 ms
Execution Time: 0.087 ms

/*
    Выводы, которые я сделала в ходе работы с индексами:
    -- чтобы индекс в принципе начал работать, в таблице должно быть достаточно большое количество строк, 
    я прицельно не тестила, сделаю это чуть позже. Из-за этого пришлось генерировать много строк для таблиц, 
    как бонус - я научилась это делать;
    -- приходится хорошо подумать прежде чем добавлять индексы на несколько столбцов, 
    тут нужно понимать явную необходимость/выгоду от таких индексов;
    -- сбор статистики с помощью ANALYSE table_name позволяет сократить косты в 2 и более раз;
    -- при создании функциональных индексов может возникать ошибка c IMMUTABLE, которую можно решить 
    добавив свою функцию с параметром IMMUTABLE;
    -- с полнотекстовым поиском были сложности: я сначала решила добавить автогенерируемый столбец, 
    как описано в документации https://postgrespro.ru/docs/postgresql/14/textsearch-tables, 
    который объединял бы поля заголовок и описание продукта, и повесила на него индекс, но с ним у 
    меня так и не получилось подцепить индекс в запросах.
*/
