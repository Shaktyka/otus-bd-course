...
1. **Адреса** (adresses)
    ...
    * _id_ - идентификатор записи
    * _dttmcr_ - дата и время создания записи
    * *object_type* - тип объекта, на который ссылается запись (свой код для пользователей, поставщиков и прозводителей)
...
1. **История смены статусов** (statuses_history)
    ...
    * *object_type* - тип сущности (кодами: пользователь, заказ и др.) 
